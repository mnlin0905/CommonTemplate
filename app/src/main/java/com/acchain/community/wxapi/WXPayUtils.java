...
public class WXPayUtils {
    ...
    public void toWXPayNotSign(Context context, String appid) {
        iwxapi = WXAPIFactory.createWXAPI(context.getApplicationContext(), null); //初始化微信api
    ...
    public void toWXPayAndSign(Context context, String appid,final String key) {
        ...
        Runnable payRunnable = new Runnable() {  //这里注意要放在子线程
            @Override
            public void run() {
                if (TextUtils.isEmpty(builder.getAppId()) ||TextUtils.isEmpty(builder.getPartnerId()) ||TextUtils.isEmpty(builder.getPrepayId())){
                    return;
                }
                PayReq request = new PayReq(); //调起微信APP的对象
                //下面是设置必要的参数，也就是前面说的参数,这几个参数从何而来请看上面说明
...
//                request.nonceStr = genNonceStr();
                request.nonceStr = "5Cq8rlXAV0lQfEz5";
//                request.timeStamp = String.valueOf(genTimeStamp());
                request.timeStamp = "1519726620";
                request.sign = builder.getSign();
                //签名
